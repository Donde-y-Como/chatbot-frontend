...
export type OnSaveBookingFunctionParams = {
  clientId: string
  participants: number
  date: Date
  notes?: string
...
interface EventBookingModalProps {
  ...
  open: boolean
  onClose: () => void
  onSaveBooking: (data: OnSaveBookingFunctionParams) => Promise<void>
            ...
            <div className='flex flex-col gap-2'>
              ...
              <Controller
                ...
                render={({ field }) => {
                  ...
                  const maxAllowed =
                    availableSpots !== null
                      ? Math.min(availableSpots, event!.capacity.maxCapacity!)
                      : undefined
                          ...
                          <div className='text-sm text-muted-foreground'>
                            {availableSpots !== null && availableSpots > 0 ? (
                              <span className='text-blue-600'>
                                ...
                                {availableSpots !== 1 ? 'es' : ''} disponible
                                {availableSpots !== 1 ? 's' : ''} para esta
                                fecha
                              </span>
                            ) : (
            ...
            <AnimatePresence>
              {bookingsForSelectedDate.map((booking: Booking) => {
                const client = clients.find(
                  (c: ClientPrimitives) => c.id === booking.clientId
                )
                return (
                  <motion.div
                    key={booking.id}
