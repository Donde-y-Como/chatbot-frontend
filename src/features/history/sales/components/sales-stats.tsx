...
import { Banknote, CreditCard, ArrowRightLeft, FileText, TrendingUp, Package, Users } from 'lucide-react'
...
export function SalesStats({ sales }: SalesStatsProps) {
  ...
  const totalRevenue = sales.reduce((sum, sale) => sum + sale.totalAmount.amount, 0)
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      {Object.keys(paymentStats).length > 0 && (
        <Card className="md:col-span-2 lg:col-span-4">
          <CardHeader>
            <CardTitle className="text-sm font-medium">Distribución por Método de Pago</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              {Object.entries(paymentStats).map(([method, stats]) => (
                <div key={method} className={`p-4 rounded-lg ${getPaymentColor(method as PaymentMethod)}`}>
