...
import {
  usePendingServices,
  useScheduleClientServices,
} from '../hooks/usePendingServices'
...
export function PendingServicesTab({
  clientId,
  clientName,
}: PendingServicesTabProps) {
  const [selectedServices, setSelectedServices] = useState<Set<string>>(
    new Set()
  )
  const [appointmentDialogOpen, setAppointmentDialogOpen] = useState(false)
  const [serviceIdsForAppointment, setServiceIdsForAppointment] = useState<
    string[]
  >([])

  const {
    data: pendingServicesResponse,
    isLoading,
    error,
    refetch,
  } = usePendingServices(clientId)
  ...
  const pendingServices = allServices.filter(
    (service) => service.serviceItem.pendingCount > 0
  )
  ...
  const handleAppointmentCreated = async (appointmentId: string) => {
    ...
    serviceIdsForAppointment.forEach((serviceId) => {
      const service = pendingServices.find(
        (s) => s.serviceItem.itemId === serviceId
      )
      if (service) {
    ...
    return (
      <Card>
        <CardContent className='flex flex-col items-center justify-center py-12 text-center'>
          ...
          <CardDescription>
            Todos los servicios de este cliente han sido agendados
            completamente.
          </CardDescription>
        </CardContent>
      </Card>
  ...
  const selectableServices = pendingServices.filter(
    (service) => service.serviceItem.pendingCount > 0
  )
  const allSelected =
    selectedServices.size === selectableServices.length &&
    selectableServices.length > 0
  const someSelected = selectedServices.size > 0

  // Calculate total pending appointments
  const totalPendingAppointments = pendingServices.reduce(
    (total, service) => total + service.serviceItem.pendingCount,
    0
  )
        ...
        <div className='flex items-center space-x-3'>
          ...
          <div>
            <p className='font-medium'>
              {pendingServices.length} servicio
              {pendingServices.length !== 1 ? 's' : ''} pendiente
              {pendingServices.length !== 1 ? 's' : ''}
            </p>
            <p className='text-sm text-muted-foreground'>
              {totalPendingAppointments} cita
              {totalPendingAppointments !== 1 ? 's' : ''} por agendar
              {someSelected && (
                <span className='ml-2'>
                  • {selectedServices.size} seleccionado
      ...
      <div className='space-y-3'>
        {pendingServices.map((service) => {
          ...
          const orderDate = format(
            new Date(service.orderCreatedAt),
            'dd/MM/yyyy',
            { locale: es }
          )
                ...
                <div className='flex items-start justify-between'>
                  <div className='flex items-start space-x-3'>
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        handleServiceSelection(serviceKey, !!checked)
                      }
                    ...
                    <div className='flex-1'>
                      <CardTitle className='text-base flex items-center gap-2'>
                        ...
                        {service.serviceItem.quantity > 1 && (
                          <Badge
                            variant={fullyScheduled ? 'default' : 'secondary'}
                            className='text-xs'
                          >
                            {service.serviceItem.scheduledCount}/
                            {service.serviceItem.quantity}
                          </Badge>
                        )}
                      </CardTitle>
                      ...
                      {service.serviceItem.quantity > 1 && (
                        <div className='mt-2 space-y-1'>
                          ...
                          <p className='text-xs text-muted-foreground'>
                            {service.serviceItem.pendingCount} pendiente
                            {service.serviceItem.pendingCount !== 1 ? 's' : ''}
                            {service.serviceItem.appointmentIds.length > 0 && (
                              <span className='ml-2'>
                                • {service.serviceItem.appointmentIds.length}{' '}
                                cita
                                {service.serviceItem.appointmentIds.length !== 1
                                  ? 's'
                                  : ''}{' '}
                                creada
                                {service.serviceItem.appointmentIds.length !== 1
                  ...
                  <div className='text-right'>
                    <p className='font-semibold'>
                      {formatPrice(service.serviceItem.finalPrice)}
                    </p>
                    <p className='text-sm text-muted-foreground'>
              ...
              <CardContent className='pt-0'>
                <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3'>
                  ...
                  <Button
                    ...
                    disabled={
                      scheduleServicesMutation.isPending || !canScheduleMore
                    }
                  ...
                  >
                    ...
                    {canScheduleMore
                      ? service.serviceItem.quantity === 1
                        ? 'Agendar'
                        : `Agendar (${service.serviceItem.pendingCount})`
                      : 'Agendado'}
                  </Button>
                </div>
