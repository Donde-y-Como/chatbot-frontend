...
import {
  ...
  useGetProductTags,
  useGetUnits,
} from '../hooks/useGetAuxiliaryData'
...
export function ProductViewDialog() {
  ...
  const productCategories = categories.filter(
    (c) => selectedProduct.categoryIds?.includes(c.id) && !c.parentCategoryId
  )
  ...
  if (selectedProduct.subcategoryIds) {
    for (const subcategoryId of selectedProduct.subcategoryIds) {
      for (const parentCategory of categories.filter(
  ...
  const formatCurrency = (
    priceObj: { amount: number; currency: string } | number
  ) => {
  ...
  const priceWithTaxes =
    finalPrice.amount * (1 + (selectedProduct.taxes || 0) / 100)
  const margin =
    selectedProduct.price.amount > 0
      ? ((selectedProduct.price.amount - selectedProduct.cost.amount) /
          selectedProduct.price.amount) *
        100
      : 0
              ...
              <CardContent>
                <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
                  ...
                  <div className='text-center'>
                    ...
                    <p className='text-2xl font-bold'>
                      {selectedProduct.minimumInventory}
                    </p>
                    <p className='text-xs text-muted-foreground'>
                      {unit?.abbreviation}
                    </p>
                  ...
                  <div>
                    ...
                    <div className='flex flex-wrap gap-2'>
                      {productSubcategories.map((subcategory) => {
                        ...
                        const displayName = parentCategoryName
