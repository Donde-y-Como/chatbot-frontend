...
const getStockBadge = (stock: number, minimumInventory: number) => {
  ...
  if (stock <= minimumInventory) {
    return (
      <Badge
        variant='outline'
        className='text-sm bg-yellow-50 text-yellow-800 border-yellow-200'
      >
        Stock bajo ({stock})
      </Badge>
    )
...
export const createProductColumns = (
...

  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className='flex flex-col'>
          <span className='font-medium text-sm'>{formatPrice(cost)}</span>
        </div>
      )
    },
    enableSorting: true,
        ...
        const categoryItems = categoryIds
          .map((categoryId) => {
            const category = categories.find(
              (c) => c.id === categoryId && !c.parentCategoryId
            )
            return category
              ? {
                  id: categoryId,
